# coding: utf-8

"""
    NexusRAG API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: v1
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from nexusrag_sdk.models.usage_timeseries_response import UsageTimeseriesResponse

class TestUsageTimeseriesResponse(unittest.TestCase):
    """UsageTimeseriesResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> UsageTimeseriesResponse:
        """Test UsageTimeseriesResponse
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `UsageTimeseriesResponse`
        """
        model = UsageTimeseriesResponse()
        if include_optional:
            return UsageTimeseriesResponse(
                metric = '',
                granularity = '',
                points = [
                    { }
                    ]
            )
        else:
            return UsageTimeseriesResponse(
                metric = '',
                granularity = '',
                points = [
                    { }
                    ],
        )
        """

    def testUsageTimeseriesResponse(self):
        """Test UsageTimeseriesResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
